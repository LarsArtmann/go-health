...
| [docs/detailed-checks-cookbook.md](docs/detailed-checks-cookbook.md)                             | How-to: per-check durations on the wire — `NewChecks` (free), `NewWithDetailedCheck` (self-timed), `DetailedHealthRecorder` (recorder upgrade); raw injector path cannot report                        |
...
| [docs/adr/](docs/adr/)                                                                           | Architecture decision records: stdlib errors (001), zero logging (002), three-state classify (003), recorder decoupling (004), aggregate source names (005), duration units split by granularity (006) |

...
1. **Make the pre-tag staleness grep a mandatory release step** — grep README/CONTRIBUTING/FEATURES/AGENTS for: stale version claims, stale toolchain claims, "stability" lines, "unreleased" language. Better: enforce it (see §f-9). Docs freeze at tag; every doc fix after tagging is a fix for the _next_ release only.
...
_Impact-sorted within groups. [O] = owner-gated / owner action. Items 1–16 follow directly from this session; 17–50 are grounded in session observations + still-open items from the 21-01 report._

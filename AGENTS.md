...
- **Throttled live eval is the only path that stores into `latest` outside the refresh loop** — a live-mode probe (`WithRefreshInterval(0)`) without `WithLiveThrottle` never populates its cache, so `CachedResponse()` returns the zero `Response` and an aggregate over such sources merges emptiness (pinned by `TestCachedResponse_NeverStartedSource`; test fixtures must prime via a throttled readiness request, see `newPrimedSource`).

...
| Consumer verification        | FULLY_FUNCTIONAL | `go-health-dashboard` compiles and tests green against released v0.1.2 (no replace directive) and against HEAD; `auditlog.Plugin` passes `var _ health.HealthRecorder` and runs a live probe end-to-end. |
...
| Remote federation over HTTP | FULLY_FUNCTIONAL | `health/federation` pulls N deployed instances' health documents and merges them: namespaced `name/check` keys, worst-of roll-up, `name/reachable` fail row on unreachable remotes, per-remote startup latches, per-fetch timeout + 1 MiB body cap + status validation on untrusted input. `federation/federation.go`, docs/federation-design.md |
...
| Feature                                | Status           | Notes                                                                                                                                                                                                                         |
| -------------------------------------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Status enum (pass/fail/warn)           | FULLY_FUNCTIONAL | String type with three constants; frozen — see docs/starting-status-design.md. `Status.Rank()` exposes the severity ordering (fail 0 < warn 1 < pass 2, unknown ranks as pass) shared by the merge sites. `types.go`                                                                     |
...
| Gate apps (`.#gates`, `.#ci-emulation`, `.#fuzz-long`) | FULLY_FUNCTIONAL | One-command pre-push sweep mirroring CI; go-free-PATH emulation; weekly 5-min fuzz budget with its own scheduled workflow. All verified end-to-end 2026-09-04. `flake.nix`, `.github/workflows/fuzz-long.yml`                                  |
...
| CI pipeline                                            | FULLY_FUNCTIONAL | GitHub Actions (`.github/workflows/ci.yml`): test-race + short fuzz, vet+lint, govulncheck+gosec, flake check, OpenAPI spec. Nix runners, SHA-pinned actions, goPkg toolchain pinned. Green on `master` and on the `v0.1.1`/`v0.1.2` tag runs. |
...
| ADRs                                                   | FULLY_FUNCTIONAL | ADR-001..004 (stdlib errors, zero logging, three-state classify, HealthRecorder decoupling) + design notes under docs/.                                                                                                                        |
